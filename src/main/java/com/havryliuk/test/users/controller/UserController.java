...
public class UserController implements UserControllerSwaggerDescriptor {
    ...
    public void createUser(@Validated({FieldsRequired.class}) @RequestBody DataUserDto user, HttpServletResponse response) {
        log.info("POST {}", USERS_URL);
        String id = userService.create(user);
        response.addHeader(LOCATION_HEADER, USERS_URL + "/" + id);
    }

    @Override
    @PatchMapping("/{id}")
    @ResponseStatus(HttpStatus.OK)
    public void updateUserFields(@Validated({OptionalFields.class}) @RequestBody DataUserDto user, @PathVariable String id) {
        log.info("PATCH {}", USERS_URL);
